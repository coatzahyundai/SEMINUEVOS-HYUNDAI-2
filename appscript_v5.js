...
function getHeaderMap(sheet) {
  ...
  return {
    ...
    idx: {
      ...
      dacion: findColIndex(["dacion", "dacionenpago", "engancheneutro", "dacionpago", "dacion_pago"]),
...
function saveInitialRecord(formData) {
  ...
  try {
    ...
    var folderName = (formData.cliente || "Sin Cliente") + " - " + (formData.asesor || "Sin Asesor"); 
...
function doGet(e) {
  return ContentService.createTextOutput("Servicio Activo. Script Robusto v7 (Solo Fila 2, Coincidencia Difusa).").setMimeType(ContentService.MimeType.TEXT);
}
