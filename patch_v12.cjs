...
const newSaveRecord = `function saveRecord(formData) {
...
code = code.substring(0, code.indexOf('function saveRecord(formData)')) + 
       newSaveRecord + '\n\n' + 
       code.substring(code.indexOf('function checkVIN(vin)'));

code = code.replace("Servicio Activo. Script Robusto v11.", "Servicio Activo. Script Robusto v12.");

fs.writeFileSync('appscript_v12.js', code);
