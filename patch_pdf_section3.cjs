...
const regex = /addSection\('Hojalatería y Pintura \(HYP\)', valData\.hyp, valData\.subtotal_hyp\);[\s\S]*?doc\.text\(`Gran Total: \$` \+ \(valData\.gran_total \|\| 0\)\.toFixed\(2\), 140, startY \+ 10\);/;

code = code.replace(
  /body: items\.map\(i => \[i\.descripcion, `\$` \+ i\.importe\]\),[\s\S]*?theme: 'grid',[\s\S]*?headStyles: \{ fillColor: \[0, 44, 95\] \}/g,
...
code = code.replace(
  /doc\.text\(`Subtotal General: \\\$` \+ sub\.toFixed\(2\), 140, startY\);[\s\S]*?doc\.text\(`Gran Total: \\\$` \+ \(valData\.gran_total \|\| 0\)\.toFixed\(2\), 140, startY \+ 10\);/,
